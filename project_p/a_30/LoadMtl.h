...
void read_mtl_file(const std::string& filename, const std::string path, std::unordered_map<std::string, Material>& materials) {
    ...
    while (std::getline(file, line)) {
        ...
        if (prefix == "newmtl") {
            ...
            ss >> materials[currentMaterial].d;
        }
        else if (prefix == "Tr") {
            ss >> materials[currentMaterial].Tr;
        }
        else if (prefix == "Tf") {
            ss >> materials[currentMaterial].Tf.x >> materials[currentMaterial].Tf.y >> materials[currentMaterial].Tf.z;
        }
        else if (prefix == "illum") {
            ss >> materials[currentMaterial].illum;
        }
            ...
            Texture transparencyTexture = load_texture(path + texturePath, "transparency");
            if (transparencyTexture.id != 0) {
                materials[currentMaterial].textures.push_back(transparencyTexture);
                materials[currentMaterial].map_d = texturePath;
            }
            else {
                std::cerr << "[ERROR] Failed to load transparency texture: " << texturePath << std::endl;
            }
        }
    }
